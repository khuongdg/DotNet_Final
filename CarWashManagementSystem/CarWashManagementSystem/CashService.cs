    ...
    public partial class CashService : Form
    {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow dr in dgvService.Rows)
            {
                ...
                if (chkbox)
                {
                    try
                    {
                        cm = new SqlCommand("IF NOT EXISTS (SELECT * FROM Cash WHERE sid=@sid AND transno=@transno) INSERT INTO Cash(transno,cid,sid,vid,price,date) VALUES(@transno,@cid,@sid,@vid,@price,@date)", dbcon.connect());
        ...
        public void loadService()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT * FROM Service WHERE name LIKE '%" + txtSearch.Text + "%'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
