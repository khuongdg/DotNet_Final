    ...
    public partial class Setting : Form
    {
        ...
        private void dgvVehicleType_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete") // if you want to delete the record to click the delete icon on the datagridview
            {
                try
                {
                    ...
                    {
                        cm = new SqlCommand("DELETE FROM VehicleType WHERE id LIKE'" + dgvVehicleType.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", dbcon.connect());
                        dbcon.open();
                        cm.ExecuteNonQuery();
                        dbcon.close();
        ...
        public void loadVehicleType()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT * FROM VehicleType WHERE CONCAT (name,class) LIKE '%" + txtSearchVT.Text + "%'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        private void dgvCostofGoodSold_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "DeleteCoG") // if you want to delete the record to click the delete icon on the datagridview
            {
                try
                {
                    if (MessageBox.Show("Are you sure you want to delete this record?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        cm = new SqlCommand("DELETE FROM CostofGood WHERE id LIKE'" + dgvCostofGoodSold.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", dbcon.connect());
                        dbcon.open();
                        cm.ExecuteNonQuery();
                        dbcon.close();
        ...
        public void loadCostofGood()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT * FROM CostofGood WHERE CONCAT (costname,date) LIKE '%" + txtSearchCoG.Text + "%'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        public void loadCompany()
        {
            ...
            {
                dbcon.open();
                cm = new SqlCommand("SELECT * FROM Company", dbcon.connect());
                dr = cm.ExecuteReader();
                dr.Read();
                if (dr.HasRows)
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {// now we create a function for execute querry only one line 
                    if (hasdetail)
                    {
                        dbcon.executeQuery("UPDATE Company SET name='" + txtComName.Text + "', address='" + txtComAddress.Text + "'");
                    }
                    else
                    {
                        dbcon.executeQuery("INSERT INTO Company (name,address) VALUES ('" + txtComName.Text + "','" + txtComAddress.Text + "')");
                    }
