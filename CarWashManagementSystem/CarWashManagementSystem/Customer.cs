    ...
    public partial class Customer : Form
    {
        ...
        public void loadCustomer()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT C.id,C.name, phone, carno, carmodel, V.name, address, points FROM Customer AS C INNER JOIN VehicleType AS V ON C.vid=V.id WHERE CONCAT (C.name,carno,carmodel,address) LIKE '%" + txtSearch.Text + "%'", dbcon.connect());
                dbcon.open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        {
            int i = 0;
            cm = new SqlCommand("SELECT id FROM VehicleType WHERE name LIKE '" + str + "' ", dbcon.connect());
            dbcon.open();
            dr = cm.ExecuteReader();
            dr.Read();
            if (dr.HasRows)
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete") // if you want to delete the record to click the delete icon on the datagridview
            {
                try
                {
                    ...
                    {
                        cm = new SqlCommand("DELETE FROM Customer WHERE id LIKE'" + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", dbcon.connect());
                        dbcon.open();
                        cm.ExecuteNonQuery();
                        dbcon.close();
