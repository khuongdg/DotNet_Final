        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        dbcon.executeQuery("UPDATE Cash SET status='Sold',price='" + cash.dgvCash.Rows[i].Cells[9].Value.ToString() + "' WHERE id='" + cash.dgvCash.Rows[i].Cells[1].Value.ToString() + "'");
                        dbcon.executeQuery("UPDATE Customer SET points += " + 1 + " WHERE id='" + cash.customerId + "'");
                    }
