    ...
    public partial class Report : Form
    {
        ...
        public void loadTopSelling()
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT TOP 10 se.name,count(ca.sid) AS qty, ISNULL(SUM(ca.price),0) AS total FROM Cash AS ca JOIN Service AS se ON ca.sid=se.id WHERE ca.date BETWEEN '" + dtFromTopSelling.Value.ToString() + "' AND '" + dtToTopSelling.Value.ToString() + "' AND status LIKE 'Sold' " +
        ...
        public void loadRevenus()
        {
            ...
            try
            {
                ...
                double total = 0;
                cm = new SqlCommand("SELECT date,ISNULL(sum(price),0) AS total FROM Cash WHERE date BETWEEN '" + dtFromRevenud.Value.ToString() + "' " +
        ...
        {
            txtRevenus.Text = extractData("SELECT ISNULL(SUM(price),0) AS total FROM Cash WHERE date BETWEEN '" + dtFromGP.Value.ToString() + "' AND '" + dtToGP.Value.ToString() + "'").ToString("#,##0.00");
